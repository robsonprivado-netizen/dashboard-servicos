...
async function fetchSheet(){
  ...
  const lines=(data.values||[]).map(r=>r.map(v=>(v===undefined||v===null)?"":String(v).trim()));
...
function parseData(lines){
  ...
  lines.forEach(row=>{
    ...
    targets.forEach(t=>{
      if(name===t&&!metrics[t]){
        metrics[t]={values:rw.map(({col,week})=>({week,value:row[col]||"0"})),wow:row[109]||"",vsMeta:row[111]||""};
