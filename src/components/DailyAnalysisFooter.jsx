...
const DAILY_SYSTEM_PROMPT = `Você é um analista estratégico sênior de um marketplace de serviços para casa.
Produza análise executiva de performance diária para reunião de liderança.
...
function buildDailyContext(dailyData, dailyChartData) {
  if (!dailyData || !dailyChartData.length) return "";
  const lastDate = dailyData.dates[dailyData.dates.length - 1];
  const last7 = dailyChartData.slice(-7);
  const prev7 = dailyChartData.slice(-14, -7);
  const wsum = (arr, k) => arr.reduce((s, d) => s + (d[k] || 0), 0);
  const wowPct = (k) => {
    const c = wsum(last7, k), p = wsum(prev7, k);
    return p > 0 ? ((c - p) / p * 100).toFixed(1) : null;
  };

  const METRICS = [
    { key: "GMV Total",    apiKey: "GMV TOTAL" },
    { key: "App",          apiKey: "GMV App" },
    { key: "Site",         apiKey: "GMV Site" },
    { key: "GuideShops",   apiKey: "GMV GuideShops" },
    { key: "TDV",          apiKey: "GMV TDV" },
    { key: "Avulso",       apiKey: "GMV AVULSO TOTAL" },
    { key: "Conversão %",  apiKey: "CONVERSÃO GERAL (BUNDLE)" },
    { key: "AOV",          apiKey: "AOV TOTAL" },
  ];

  let s = `DADOS DIÁRIOS PERFORMANCE SERVIÇOS 2026\nPeríodo: ${dailyData.dates[0]} a ${lastDate}\n\n`;

  for (const { key, apiKey } of METRICS) {
    const row = dailyChartData.map(d => `${d.date}=${d[key] || 0}`).join(", ");
    const wow = wowPct(key);
    const meta = dailyData.vsMeta?.[apiKey];
    s += `${apiKey}: ${row}`;
    if (wow != null) s += ` | WoW: ${parseFloat(wow) >= 0 ? "+" : ""}${wow}%`;
    if (meta) s += ` | vs Meta: ${meta}`;
    s += "\n";
  }

  s += "\nCONTEXTO: Marketplace serviços para casa. Canais: App/Site (digital), GuideShops (loja física), TDV (vendedor dedicado), Avulso (montagem, limpeza, impermeabilização).";
  return s;
}
...
export default function DailyAnalysisFooter({ dailyData, dailyChartData }) {
  ...
  async function generate() {
    ...
    setErrorMsg("");
    try {
      const context = buildDailyContext(dailyData, dailyChartData);
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: `Analise os dados diários e retorne o JSON:\n\n${context}` }]
        })
      });
  ...
  return (
    <div style={{ borderTop: "1px solid #22283a", marginTop: 40, paddingTop: 36 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.5px" }}>Análise Executiva Diária</h2>
        ...
        <button onClick={generate} disabled={status === "loading"} style={{ display: "flex", alignItems: "center", gap: 8, background: `linear-gradient(135deg,${C.green},#00b37e)`, color: "#0a0c10", border: "none", padding: "11px 22px", borderRadius: 8, fontWeight: 700, fontSize: 14, cursor: status === "loading" ? "not-allowed" : "pointer", opacity: status === "loading" ? 0.6 : 1 }}>
          {status === "loading" ? "⟳ Gerando..." : status === "done" ? "◈ Regenerar" : "◈ Gerar Análise Diária"}
        </button>
      </div>
      ...
      {status === "loading" && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, padding: 48, color: "#6b7280", fontFamily: "'DM Mono',monospace", fontSize: 13 }}>
          ...
          Analisando dados de performance diária...
